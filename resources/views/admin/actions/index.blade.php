...
@section('content_header')
    <div class="d-flex justify-content-between">
        <h1>Ações Previdenciárias</h1>
        @can('create-action')
            <a href="{{ route('admin.actions.create') }}" class="btn btn-md btn-info" title="Adicionar novo registro">
            ...
            <table class="table table-striped table-hover table-head-fixed" id="myTable" data-order='[[ 1, "asc" ]]'
                ...
                <tbody>
                    @foreach ($actions as $action)
                        <tr>
                            <td class="py-2">{{ $action->name }}</td>
                            <td class="py-2">{{ \Carbon\Carbon::parse($action->updated_at)->format('d/m/Y H:m:s') }}</td>
                            <td class="py-2 px-2">
                                <div class="btn-group">
                                    @can('edit-action')
